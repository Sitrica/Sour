package com.sitrica.bungeecord.core.utils;
...
public class Utils {
	...
	public static void loadClasses(Plugin instance, String basePackage, String... subPackages) {
		for (int i = 0; i < subPackages.length; i++) {
			subPackages[i] = subPackages[i].replace('.', '/') + "/";
		}
		JarFile jar;
		try {
			jar = new JarFile(instance.getFile());
		} catch (IOException e) {
			e.printStackTrace();
			return;
		}
