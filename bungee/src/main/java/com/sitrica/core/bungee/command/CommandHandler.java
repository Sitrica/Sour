...
import com.sitrica.core.bungee.messaging.MessageBuilder;
...
public class CommandHandler extends Command {
	...
	public CommandHandler(SourBungeePlugin instance, String main, String permission, AbstractCommand command, String... commandPackages) {
		...
		Utils.getClassesOf(instance, AbstractCommand.class, commandPackages).forEach(clazz -> {
			try {
				subcommands.add(clazz.getConstructor(SourBungeePlugin.class).newInstance(instance));
	...
	private void processRequirements(AbstractCommand command, CommandSender sender, String[] arguments) {
		...
		if (command.getPermissionNodes() == null || Arrays.stream(command.getPermissionNodes()).parallel().anyMatch(permission -> sender.hasPermission(permission))) {
			...
			AbstractCommand.ReturnType returnType = command.runCommand(entered, sender, array);
